...
import { Card, Alert, Spinner, Modal } from "flowbite-react";
...
export default function StructureDetail() {
  ...
  const renderNodes = (nodes) => {
    if (!nodes || nodes.length === 0) {
      return <p className="text-gray-500">No nodes in this structure.</p>;
    }

    return (
      <div className="space-y-2">
        {nodes.map((node, index) => (
          <div key={node.id || index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
            <div>
              <span className="font-medium">{node.label}</span>
              {node.parent_id && (
                <span className="text-sm text-gray-600 ml-2">(Parent ID: {node.parent_id})</span>
              )}
            </div>
          </div>
        ))}
